...
for pkg in digraphs json profiling NormalizInterface simpcomp; do
  VERSION_PKG_DIR=`find . -maxdepth 1 -iname "$pkg-*" -type d`
...
if [[ "$target_platform" == "osx-64" ]]; then
    ${INSTALL_NAME_TOOL} -change ${SRC_DIR}/pkg/${SEMIGROUPS_PKG_DIR:2}/libsemigroups/../bin/lib/libsemigroups.0.dylib @rpath/libplanarity.0.dylib bin/64/semigroups.so
fi
popd
